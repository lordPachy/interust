...
users_with_scores[is.na(n_downloads), n_downloads := 0]
users_with_scores[n_downloads == 0]
```
...
users_aggregate_log[km$cluster == 1 | km$cluster == 9 | km$cluster == 14]$n_being_watched_from
summary(users_aggregate_log[km$cluster == 1 | km$cluster == 9 | km$cluster == 14])
...
## Network representation: users connected by commits in the same repostitories
```{r}
users_repos
...
mean_distance(g)
ggplot(data = data.frame(x = degree_distribution(g)), aes(x)) + geom_density() + scale_x_log10() + scale_y_log10()
```
