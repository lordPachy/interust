...
users_aggregate_log = users_with_scores[, ':='(social_popularity =
                                          log(n_followers 
                                              + n_stars_received 
                                              +n_being_watched_from),
#                                          n_issue_reactions_received,
...
#                                            n_pull_request_comments_received),
                                      working_activity = 
                                            ...
                                            n_commits +
                                            n_pull_requests))]
users_aggregate = users_aggregate[social_popularity > 0 & social_activity > 0 & working_activity > 5 & is_bot.x == 'f']
users_aggregate_log = users_aggregate_log[social_popularity > 0 & social_activity > 0 & working_activity > 0 & is_bot.x == 'f']
users_aggregate
```
...
users_aggregate_log[km$cluster == 1 | km$cluster == 2 | km$cluster == 6]$n_being_watched_from
summary(users_aggregate_log[km$cluster == 1 | km$cluster == 2 | km$cluster == 6])
...
col_of_interest = 'n_followers'

#observe them
users_aggregate_log[, ..col_of_interest]

#count how many observations are needed to complete the table
n_observations = count(users_aggregate_log)
n_observations
n_partial_observations = count(users_aggregate_log[km$cluster == 1 | km$cluster == 2 | km$cluster == 6, ..col_of_interest])
as.numeric(n_partial_observations)
n_observations - n_partial_observations

#compose the data frames
comparison_lucky = data.frame(x = sample(
  as.vector(users_aggregate_log[, ..col_of_interest]), 
  as.numeric(n_partial_observations)
  ),
                              y = as.vector(users_aggregate_log[km$cluster == 1 | km$cluster == 2 | km$cluster == 6, ..col_of_interest]))
comparison_lucky$y
#plot
comparison_lucky
ggplot(comparison_lucky) +scale_color_manual(labels = c("General distribution", "Lucky people distribution"), values = c("red", "blue"))+
  geom_density(aes(x), colour = 'red') + 
  geom_density(aes(y), colour = 'blue')+ 
...
ggplot(data = df) + geom_density(mapping = aes(x = theoretical, colour = 'red')) +
  scale_color_manual(labels = c("Real distribution", "Theoretical distribution"), values = c("blue", "red")) + geom_density(mapping = aes(x = real, colour = 'blue')) + scale_x_continuous(trans = 'log10') + xlab('N of followers')
```
