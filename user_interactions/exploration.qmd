...
# Sponsorships and the distance from top users
...
rownames(distance_sponsored_topg)
distance_sponsored_topg_reversed
distance_st_min_rev = apply(distance_sponsored_topg_reversed, 1, FUN = min)
distance_st_min_rev
...
distance_si_min_rev = apply(distance_sponsored_influencers_reversed, 1, FUN = min)
distance_si_min_rev
...
```{r}
ggplot(distance_si_min_rev, aes(distance)) + geom_histogram()
```
