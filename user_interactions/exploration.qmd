...
```{r}
ggplot(users_aggregate_cluster, aes(working_activity, social_popularity, colour = cluster)) + geom_point()
```
...
## Network representation: users connected by commits in the same repositories
```{r}
users_repos
...
ggplot(usage_sample_by_timepassed, aes(timestamp_diff, ratio)) + geom_point()
```
...
distance_st_min_rev = apply(distance_sponsored_topg_reversed, 1, FUN = mean, na.rm= T)
distance_st_min_rev
...
```{r}
ggplot(distance_st_min_rev, aes(distance)) + geom_density()
```
...
distance_si_min_rev = apply(distance_sponsored_influencers_reversed, 1, FUN = mean, na.rm = T)
distance_si_min_rev
...
```{r}
ggplot(distance_si_min_rev, aes(distance)) + geom_density()
```
...
users_aggregate_model = copy(users_aggregate)
